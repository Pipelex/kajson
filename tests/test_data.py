...
class ClassWithTrickyTypes:
    ...
    def __eq__(self, other: Any) -> bool:
        logging.debug(f"Comparing {self} with {other}, i.e. {self.__dict__} with {other.__dict__}")
...
class MyRootModel(RootModel[MyDictType]):
    def check(self):
        if not self.root:
            logging.debug("self.root is empty")
        logging.debug(f"self.root.items(): {self.root.items()}")
        logging.debug(f"self.root: {self.root}")
