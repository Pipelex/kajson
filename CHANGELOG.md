...
### Fixed
- **Codebase:** Fixed an `IndexError` in `ClassRegistry.register_classes_dict()` when passing an empty dictionary; seeding with an empty collection is now a safe no-op, matching `register_classes()`. Callers that guard the call site against an empty dict can drop the guard.
- **Pipelines:** Hardened the PyPI publish GitHub Actions workflow against Sigstore outages. The GitHub Release and its distribution artifacts are now created *before* signing, so a signing failure can no longer cost a published version its release and tag; the Sigstore action is bumped to `v3.5.0` (SHA-pinned) and marked `continue-on-error: true`, emitting a warning with re-run instructions if it fails; the signature upload is scoped to the `.sigstore.json` bundles so `--clobber` cannot delete the freshly attached distributions; and the re-run that recovers a missing signature actually works — the release step edits an existing release instead of failing, and the `build` job reuses the distributions an earlier attempt already stored rather than producing a second build, so the files GitHub serves and Sigstore attests are the exact bytes PyPI published. Rebuilding could not promise that: `[build-system] requires` names no hatchling version, so a backend release landing between two attempts would change the output of the same commit. PyPI's `skip-existing` is scoped to re-runs (`github.run_attempt > 1`): on a first attempt an unbumped push to `main` must still fail at PyPI, since letting it through would carry it into the release job and replace an existing tag's artifacts with a build of a newer commit. The release job now also refuses outright to touch a release whose tag names a different commit than the one being built, which closes the same hole on the re-run path — where the attempt counter alone cannot tell a recovery from a re-run of a duplicate-version failure. That check fails closed: only a `404` or `422` from the GitHub API counts as "this version is not tagged yet", so an outage, a rate limit or a bad token stops the job instead of being read as permission to proceed.

### Changed
- **Tooling:** Pinned `ruff` to an exact version (`==0.16.4`), replacing the `>=0.6.8` floor, to ensure consistent behavior between the CLI and the VS Code extension — Ruff 0.16 lints `pyproject.toml` itself, and a pre-0.16 binary parsed it as Python source and painted phantom `invalid-syntax` diagnostics. Dev-only dependency; no shipped behavior changes.
- **Documentation:** Reformatted all Python code examples in `README.md` and `docs/` to follow the project's Ruff formatting rules (line length, trailing commas, quote styles) without altering behavior.

### Removed
- **Editor config:** Removed the invalid `ruff.configuration` setting from `.vscode/settings.json`, which passed a CLI flag (`--config=pyproject.toml`) instead of a file path and was silently ignored.
