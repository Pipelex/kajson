...
def encode_path(path: Path) -> dict:
    return {"__path__": str(path), "is_absolute": path.is_absolute()}
...
# Usage
config = {"project_root": Path("/home/user/project"), "config_file": Path("config/settings.json")}
...
class Color:
    ...
    def __json_encode__(self) -> dict:
        ...
        return {"rgb": (self.r, self.g, self.b), "name": self.name, "hex": f"#{self.r:02x}{self.g:02x}{self.b:02x}"}
...
class Matrix:
    ...
    def __json_encode__(self) -> dict:
        return {"data": self.data.tolist(), "shape": self.shape, "dtype": str(self.data.dtype)}
...
type_handlers = {
    Fraction: {"encode": lambda f: {"num": f.numerator, "den": f.denominator}, "decode": lambda d: Fraction(d["num"], d["den"])},
    IPv4Address: {"encode": lambda ip: {"ipv4": str(ip)}, "decode": lambda d: IPv4Address(d["ipv4"])},
    IPv6Address: {"encode": lambda ip: {"ipv6": str(ip)}, "decode": lambda d: IPv6Address(d["ipv6"])},
}
...
# All types now work
data = {"fraction": Fraction(3, 4), "ipv4": IPv4Address("192.168.1.1"), "ipv6": IPv6Address("2001:db8::1")}
...
if platform.system() == "Windows":
    ...
    kajson.UniversalJSONEncoder.register(WindowsPath, lambda p: {"windows_path": str(p)})
    kajson.UniversalJSONDecoder.register(WindowsPath, lambda d: WindowsPath(d["windows_path"]))
```
...
# Usage
data = {"matrix": np.array([[1, 2], [3, 4]]), "vector": np.array([1.0, 2.0, 3.0])}
...
def encode_dataframe(df: pd.DataFrame) -> dict:
    return {"data": df.to_dict(orient="records"), "columns": df.columns.tolist(), "index": df.index.tolist()}
...
# Usage
df = pd.DataFrame({"name": ["Alice", "Bob", "Carol"], "age": [25, 30, 35], "city": ["NYC", "LA", "Chicago"]})
...
def encode_complex_type(value):
    return {"__mytype__": {"version": 1, "data": value.serialize()}}
...
registry.register(Decimal, lambda d: {"decimal": str(d)}, lambda data: Decimal(data["decimal"]))
